...
class WeatherAPI:
    ...
    def get_forecast(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to fetch forecast data: HTTP {response.status_code} - {response.text}"
            )
    ...
    def get_air_quality(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to fetch air quality data: HTTP {response.status_code} - {response.text}"
            )
    ...
    def get_air_pollution_forecast(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to fetch air pollution forecast: HTTP {response.status_code} - {response.text}"
            )
    ...
    def get_coordinates(self, location: str) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to geocode location: HTTP {response.status_code} - {response.text}"
            )
    ...
    def get_daily_forecast_summary(self, lat: float, lon: float, days: int = 5) -> Dict[str, Any]:
        ...
        return {"location": forecast_data["location"], "daily_forecast": daily_summaries}
    ...
    def get_forecast_by_time_of_day(self, lat: float, lon: float, days: int = 5) -> Dict[str, Any]:
        ...
        for date in sorted(grouped_data.keys())[:days]:
            ...
            for period in ["morning", "afternoon", "evening"]:
                ...
                most_common_condition = (
                    Counter(conditions).most_common(1)[0][0] if conditions else "N/A"
                )
        ...
        return {"location": forecast_data["location"], "forecast_by_time": result}
