...
def get_sleep_metrics(db: Session) -> Dict[str, Any]:
    ...
    if not sleeps:
        return {
            ...
            "bedtime": {"last_7_days": [], "avg_7_days": None, "avg_28_days": None, "latest": None, "unit": "HH:MM"},
            "wake_time": {"last_7_days": [], "avg_7_days": None, "avg_28_days": None, "latest": None, "unit": "HH:MM"}
        }
...
def get_strain_metrics(db: Session) -> Dict[str, Any]:
    ...
    if not workouts:
        return {
            "strain": get_metric_aggregation([], "")
        }
    
    strain_values = [w.strain for w in workouts if w.strain is not None]
    
    return {
        "strain": get_metric_aggregation(strain_values, "")
    }
