    ...
    if weather_service:
        try:
            ...
            dashboard_data["context"]["weather"] = {
                ...
                "forecast_today": f"High {high_temp}°C, Low {low_temp}°C" if high_temp and low_temp else "N/A",
                "air_quality": {
                    "aqi": air_quality["aqi"],
...
async def get_weather_extended(location: str = DEFAULT_LOCATION) -> Dict[str, Any]:
    ...
    if not weather_service:
        raise HTTPException(
            status_code=503,
            detail="Weather service not configured - check OPENWEATHER_API_KEY"
        )
    ...
    try:
        ...
        daily_forecast = weather_service.get_daily_forecast_summary(coords["lat"], coords["lon"], days=5)
        
        # Time-of-day forecast (morning/afternoon/evening)
        time_of_day_forecast = weather_service.get_forecast_by_time_of_day(coords["lat"], coords["lon"], days=5)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving extended weather data: {str(e)}"
        )
