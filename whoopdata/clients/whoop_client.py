...
class Whoop:
    ...
    def _authenticate_client_credentials(self):
        ...
        if r.status_code != 200:
            self.logger.error(f"Client credentials authentication failed: {r.status_code} - {r.text}")
            ...
            def do_GET(self):
                ...
                if self.path.startswith('/callback'):
                    ...
                    if 'error' in params:
                        ...
                        self.end_headers()
                        self.wfile.write(f'<html><body><h1>Authorization failed!</h1><p>Error: {error}</p><p>{urllib.parse.unquote(error_desc)}</p></body></html>'.encode())
                    elif 'code' in params:
        ...
        while True:
            try:
                ...
                if not response.text:
                    self.logger.info(f"Received empty response from {self.data_endpoint}. Assuming no more data.")
                    break
                response_json = response.json()
            except requests.exceptions.JSONDecodeError as e:
                self.logger.error(f"JSONDecodeError: {e} - Response content: {response.text}")
                # If the response is not valid JSON, we should stop processing and return what we have or raise an error.
                # For now, let's assume an empty response is a sign to stop pagination.
                if not response.text.strip(): # Check if the response content is empty or just whitespace
                    self.logger.info("Response content is empty, stopping pagination.")
                    break
                else:
                    raise # Re-raise if it's not an empty response but still invalid JSON

            response_data += response_json["records"]

            # Handle pagination
            if "next_token" in response_json and response_json["next_token"]:
                params["nextToken"] = response_json["next_token"]
                self.logger.debug(f"Fetching next page: {response_json['next_token']}")
            else:
                break
