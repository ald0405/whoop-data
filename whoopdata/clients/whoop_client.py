...
class Whoop:
    ...
    def make_paginated_request(self, data_endpoint: str, transform_for_db: bool = True):
        ...
        while True:
            try:
                response = requests.get(self.data_endpoint, headers=headers, params=params)
                if not response.text:
                    self.logger.info(
                        f"Received empty response from {self.data_endpoint}. Assuming no more data."
                    )
                    break
                ...
                ):  # Check if the response content is empty or just whitespace
