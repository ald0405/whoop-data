...
@tool(
    "get_recovery_data",
    description="Get WHOOP recovery data with flexible filtering - supports latest, historical ranges, and comprehensive analysis",
)
async def get_recovery_data_tool(
    latest: bool = True,
    limit: int = 100,
    skip: int = 0,
    start_date: str = None,
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.AGENT_TIMEOUT_SECONDS) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error retrieving recovery data: HTTP {response.status_code} - {response.text}"
                )
...
@tool(
    "get_top_recoveries",
    description="Get the highest WHOOP recovery scores to see your best recovery days",
)
...
@tool(
    "get_sleep_data",
    description="Get WHOOP sleep data with flexible filtering - supports latest, historical ranges, and comprehensive analysis",
)
async def get_sleep_data_tool(
    latest: bool = True,
    limit: int = 100,
    skip: int = 0,
    start_date: str = None,
...
@tool(
    "get_workout_data",
    description="Get WHOOP workout data with flexible filtering - supports latest, historical ranges, and comprehensive analysis",
)
async def get_workout_data_tool(
    latest: bool = True,
    limit: int = 100,
    skip: int = 0,
    start_date: str = None,
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.AGENT_TIMEOUT_SECONDS) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error retrieving workout data: HTTP {response.status_code} - {response.text}"
                )
...
@tool(
    ...
    description="Get running workouts with TRIMP scores - supports large datasets for comprehensive analysis",
)
...
@tool(
    "get_tennis_workouts",
    description="Get tennis workouts and performance data - supports comprehensive historical analysis",
)
...
@tool(
    "get_weight_data",
    description="Get Withings weight data with flexible filtering - supports latest, historical ranges, and comprehensive analysis",
)
async def get_weight_data_tool(
    latest: bool = True,
    limit: int = 100,
    skip: int = 0,
    start_date: str = None,
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.AGENT_TIMEOUT_SECONDS) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error retrieving weight data: HTTP {response.status_code} - {response.text}"
                )
...
@tool(
    "get_weight_stats", description="Get weight statistics and trends over a specified time period"
)
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.AGENT_TIMEOUT_SECONDS) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error retrieving weight stats: HTTP {response.status_code} - {response.text}"
                )
...
@tool(
    "get_heart_rate_data",
    description="Get Withings heart rate data with flexible filtering - supports latest, historical ranges, and comprehensive analysis",
)
async def get_heart_rate_data_tool(
    latest: bool = True,
    limit: int = 100,
    skip: int = 0,
    start_date: str = None,
...
@tool(
    ...
    description="Get a comprehensive summary of all Withings health data including weight and cardiovascular metrics",
)
...
@tool(
    "get_all_recovery_data",
    description="Get comprehensive recovery data with flexible filtering - supports large date ranges",
)
...
@tool(
    "get_recovery_trends",
    description="Get weekly average recovery scores and trends - supports flexible time ranges",
)
...
@tool(
    "get_weather",
    description="Get current weather conditions for a location - useful for planning workouts and outdoor activities",
)
...
@tool(
    "get_air_quality",
    description="Get current air quality index (AQI) for a location - important for outdoor workout planning",
)
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.AGENT_TIMEOUT_SECONDS) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error retrieving air quality: HTTP {response.status_code} - {response.text}"
                )
...
@tool(
    ...
    description="Get multi-day weather forecast for a location - useful for planning training schedules",
)
async def get_weather_forecast_tool(
    location: str = settings.DEFAULT_LOCATION, days: int = 3
) -> str:
...
@tool(
    ...
    description="Get current TfL transport status for key lines (Jubilee, DLR, Elizabeth, Northern) - relevant for workout timing and stress levels",
)
...
@tool(
    ...
    description="Analyze what factors influence your recovery most - shows ranked importance with actionable thresholds",
)
...
@tool(
    ...
    description="Analyze how health metrics correlate with each other - reveals relationships between sleep, recovery, HRV, strain",
)
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if response.status_code == 200:
                ...
                return (
                    f"Error analyzing correlations: HTTP {response.status_code} - {response.text}"
                )
    except Exception as e:
...
@tool(
    "predict_recovery",
    description="Predict recovery score from sleep/HRV/strain inputs - get expected recovery with confidence interval",
)
...
@tool(
    ...
    description="Predict sleep performance score from sleep metrics - see expected sleep score",
)
async def predict_sleep_performance_tool(
    total_sleep_hours: float, rem_sleep_hours: float, awake_time_hours: float
) -> str:
...
@tool(
    "get_weekly_insights",
    description="Get automated weekly health insights - actionable recommendations and trends",
)
...
@tool(
    "detect_patterns",
    description="Detect trends and patterns in a specific metric - shows if recovery/HRV/RHR/sleep is trending up/down",
)
