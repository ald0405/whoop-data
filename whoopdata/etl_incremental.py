...
def get_latest_timestamps(db: Session) -> Dict[str, Optional[datetime]]:
    ...
    try:
        latest_weight = db.query(WithingsWeight).filter(
            WithingsWeight.datetime.isnot(None)
    ...
    try:
        latest_hr = db.query(WithingsHeartRate).filter(
            WithingsHeartRate.datetime.isnot(None)
        ).order_by(WithingsHeartRate.datetime.desc()).first()
