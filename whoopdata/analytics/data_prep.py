...
def get_recovery_with_features(
    ...
    df['light_sleep_hours'] = (df['total_time_in_bed_time_milli'] - df['total_awake_time_milli'] - 
                                df['total_rem_sleep_time_milli'] - df['total_slow_wave_sleep_time_milli']) / 3600000
    ...
    df['total_sleep_need_hours'] = (df['baseline_sleep_needed_milli'] + df['need_from_sleep_debt_milli'] + 
                                     df['need_from_recent_strain_milli']) / 3600000
    ...
    df['avg_hr_percentage_of_max'] = ((df['average_heart_rate'] / df['max_heart_rate'] * 100)
    ...
    for window in [7, 14]:
        df[f'recovery_rolling_{window}d'] = df['recovery_score'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'hrv_rolling_{window}d'] = df['hrv_rmssd_milli'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'rhr_rolling_{window}d'] = df['resting_heart_rate'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'strain_rolling_{window}d'] = df['strain'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'sleep_rolling_{window}d'] = df['sleep_hours'].shift(1).rolling(window=window, min_periods=3).mean()
    
    # ===== Rolling Variability Features (standard deviation) =====
    for window in [7, 14]:
        df[f'hrv_std_{window}d'] = df['hrv_rmssd_milli'].shift(1).rolling(window=window, min_periods=3).std()
        df[f'rhr_std_{window}d'] = df['resting_heart_rate'].shift(1).rolling(window=window, min_periods=3).std()
        df[f'recovery_std_{window}d'] = df['recovery_score'].shift(1).rolling(window=window, min_periods=3).std()
    ...
    if 'cycle_id' in df.columns and len(df) > 0:
        try:
            ...
            if len(workouts_df) > 0:
                ...
                workout_agg = workouts_df.groupby('cycle_id').agg({
                    'sport_id': lambda x: x.mode()[0] if len(x.mode()) > 0 else None,  # Most common sport
                    'workout_strain': 'sum',
                ...
                for zone in range(6):
                    ...
                    df.loc[mask, f'zone_{zone}_pct'] = (df.loc[mask, f'zone_{zone}_minutes'] / df.loc[mask, 'total_zone_minutes'] * 100)
                ...
                if 'workout_start' in df.columns:
                    df['workout_start_hour'] = pd.to_datetime(df['workout_start'], errors='coerce').dt.hour
                    df['workout_is_morning'] = (df['workout_start_hour'] >= 5) & (df['workout_start_hour'] < 12)
                    df['workout_is_afternoon'] = (df['workout_start_hour'] >= 12) & (df['workout_start_hour'] < 18)
                    df['workout_is_evening'] = (df['workout_start_hour'] >= 18) & (df['workout_start_hour'] < 22)
...
def get_sleep_quality_features(
    ...
    query = db.query(
        ...
        Recovery, Sleep.id == Recovery.sleep_id, isouter=True
    ).join(
        Cycle, Recovery.cycle_id == Cycle.id, isouter=True
    ).filter(
        Sleep.nap == False,  # Exclude naps
        Sleep.sleep_efficiency_percentage.isnot(None)
    )
    ...
    df['total_sleep_need_hours'] = (df['baseline_sleep_needed_milli'] + df['need_from_sleep_debt_milli'] + 
                                     df['need_from_recent_strain_milli']) / 3600000
    df['sleep_deficit'] = df['total_sleep_need_hours'] - df['total_sleep_hours']  # Negative = surplus
    
    # ===== Sleep Stage Ratios =====
    df['rem_percentage'] = (df['rem_sleep_hours'] / df['total_sleep_hours'] * 100).fillna(0)
    df['deep_sleep_percentage'] = (df['slow_wave_sleep_hours'] / df['total_sleep_hours'] * 100).fillna(0)
    df['light_sleep_percentage'] = (df['light_sleep_hours'] / df['total_sleep_hours'] * 100).fillna(0)
    ...
    for window in [7]:
        df[f'sleep_efficiency_rolling_{window}d'] = df['sleep_efficiency_percentage'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'sleep_hours_rolling_{window}d'] = df['total_sleep_hours'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'bedtime_rolling_{window}d'] = df['bedtime_hour'].shift(1).rolling(window=window, min_periods=3).mean()
        df[f'strain_rolling_{window}d'] = df['strain'].shift(1).rolling(window=window, min_periods=3).mean()
    
    # ===== Bedtime Consistency =====
    df['bedtime_std_7d'] = df['bedtime_hour'].shift(1).rolling(window=7, min_periods=3).std()
    df['bedtime_consistency_score'] = (100 - df['bedtime_std_7d'] * 20).clip(0, 100)  # Lower std = better consistency
    ...
    df['has_rolling_features'] = (~df[['sleep_efficiency_rolling_7d', 'sleep_hours_rolling_7d']].isna()).all(axis=1)
...
def get_training_data(
    ...
    if len(X) == 0:
        raise ValueError(f"No valid samples found after extracting features. DataFrame has {len(df)} rows but features resulted in 0 samples.")
