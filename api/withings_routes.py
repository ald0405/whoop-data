...
@router.get("/withings/weight", response_model=List[dict])
async def get_weight_data(
    ...
    """
    Get Withings weight and body composition data
    """
    try:
        ...
        records = query.order_by(WithingsWeight.datetime.desc()).limit(limit).all()
...
@router.get("/withings/heart-rate", response_model=List[dict])
async def get_heart_rate_data(
    ...
    """
    Get Withings heart rate and blood pressure data
    """
    try:
        ...
        records = query.order_by(WithingsHeartRate.datetime.desc()).limit(limit).all()
